...
const remove = async () => {
  ...
  await run(extensionsPath, 'yarn','unlink', 'coc-sourcekit')

  const pkg = await readJSON(pkgPath)
  delete pkg.dependencies['coc-sourcekit']
  await writeJSON(pkgPath, pkg)

  await run(extensionsPath, 'yarn','add', 'coc-sourcekit')
}

const add = async () => {
  const pkg = await readJSON(pkgPath)
  pkg.dependencies['coc-sourcekit'] = '*'
  await writeJSON(pkgPath, pkg)

  await run('.', 'yarn','link')
  await run(extensionsPath, 'yarn','link', 'coc-sourcekit')
}
